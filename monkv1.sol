 ...
 * @notice Raffles automatically restart after each draw
 * @notice Each raffle lasts 1 hour
 */
contract RaffleLendingProtocol is ReentrancyGuard, Ownable {
    ...
    // ✨ CHANGED: Raffle duration is now 1 hour (3600 seconds)
    uint256 public constant RAFFLE_DURATION = 1 hours;
    ...
    // ✨ MODIFIED: Public function for manually starting first raffle
    function startNewRaffle(uint256 numberOfWinners) external onlyOwner {
        ...
        _startNewRaffle(numberOfWinners);
    }
    
    // ✨ NEW: Internal function that can be reused for auto-restart
    function _startNewRaffle(uint256 numberOfWinners) internal {
        raffleCounter++;
    ...
    // ✨ MODIFIED: Raffle now auto-restarts after drawing
    function drawRaffle(uint256 raffleId) external onlyOwner {
        ...
        // ✨ AUTO-RESTART: Automatically start the next raffle with same number of winners
        _startNewRaffle(raffle.numberOfWinners);
    }
    
    // View functions
    function getUserDepositInfo(address user, address asset) external view returns (uint256 amount, uint256 tickets) {
        UserDeposit memory userDepositData = userDeposits[user][asset];
        return (userDepositData.amount, userDepositData.raffleTickets);
    }
    ...
    function getUserBorrowInfo(address user, address asset) external view returns (
        ...
        UserBorrow memory userBorrowData = userBorrows[user][asset];
        uint256 calculatedInterest = calculateBorrowInterest(user, asset);
        return (userBorrowData.amount, calculatedInterest, userBorrowData.collateralAmount, userBorrowData.collateralAsset);
    }
    ...
    function calculateBorrowInterest(address user, address asset) public view returns (uint256) {
        UserBorrow memory userBorrowData = userBorrows[user][asset];
        if (userBorrowData.amount == 0) return 0;
        
        Pool memory pool = pools[asset];
        uint256 timeElapsed = block.timestamp - userBorrowData.borrowTime;
        uint256 annualInterest = (userBorrowData.amount * pool.borrowRate) / BASIS_POINTS;
